...
export default function FetchWithUseEffectFixed({ category }: { category: string }) {
  ...
  //set to true to avoid blinking no posts state
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string>();

  useEffect(() => {
    //key to prevent race requests
    let ignore = false;
    ...
    async function fetchPosts() {
      try {
        ...
        if (!ignore) {
          setPosts(data);
          //error to undefined and posts to undefined to avoid showing wrong data when change tabs too fast
          setError(undefined);
        }
      } catch (error) {
        if (!ignore) {
